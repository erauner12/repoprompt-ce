...
- [x] 10.3 Run one live CE narrative: Director proposes revision → Needs You → Revise plan → trusted revision flow → concrete plan → user approval → execution resumes. PASS: S8 artifact `tmp/director-e2e-runs/20260711T034319Z-s8-6da7a2` reached all required checkpoints through `completed` with required events and receipts.
