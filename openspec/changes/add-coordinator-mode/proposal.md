...
- Specify delegated-run guardrails: approved concrete Mission Plan requirement, atomic rendered-checkpoint approval before runtime resume, pre-approval planning exceptions, `maxConcurrent` flight cap, explicit worktree isolation for mutable Coordinator work, terminal-state honesty, node-status monotonicity, and childAsk:auto ledger requirements.
