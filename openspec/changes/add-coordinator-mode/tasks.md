...
- [x] 13.7 App-owned Stop and terminal monotonicity: keep Stop as an app/external user-owned terminal action, accept stale Stop safely, cancel active work, clear pending asks/follow-through, and prevent terminal Mission/node states from reopening through runtime updates. Automated authority coverage is complete. Live S7 passed at `tmp/director-e2e-runs/20260711T003239Z-s7-7b0b5d`; a two-Mission target check selected completed Mission B (`12137E74-FAC1-48E8-B9F4-8A611EB60FA9`), stopped only awaiting-approval Mission A (`5587DDE5-BD9A-4306-BCC2-90C9258464D5`), preserved B as completed/selected, and confirmed repeated Stop is idempotent.
- [x] 13.8 Durable post-approval handoff: persist a post-approval continuation with the approval transaction, await a generation-aware persistence barrier before any runtime dispatch, expose pending/deferred/dispatching/delivered/failed/invalidated state in Mission status, avoid repeated-busy attempt churn, revalidate continuation authority at enqueue time, and deliver one accepted same-process continuation at the next ordinary Coordinator boundary without a second external submit. Focused production-path coverage covers the generation-aware flush and post-flush races, enqueue-time authority revalidation, hidden ordinary-boundary exactly-once lifecycle, and same-process decoded pending/deferred/dispatching recovery. Live S4 passed at `tmp/director-e2e-runs/20260711T003030Z-s4-2d9437`; full restart recovery remains explicitly deferred to S8.
