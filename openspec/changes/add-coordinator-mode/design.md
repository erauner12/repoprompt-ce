...
The classifier is the safety line. It holds when proactive follow-through is disabled, when the Coordinator is active, when any owned child is in `Needs you` or `Blocked`, or when a required human-review packet has not been acknowledged. It may resume for child terminal states, advisory review packets, and cleared continuation gates. The first implemented gate type is `reviewRequired`, cleared by `Mark reviewed`; the model also allows a future `actionApprovalRequired` gate that names exactly one approved next action such as `commitChanges`, `createPullRequest`, or `applyMergePreview`.

`Mark reviewed` means exactly one thing: the human review gate is cleared. It is not approval to apply, merge, commit, push, or bypass any remaining permission prompt. A future scoped action approval may clear a different gate, but only for the named action; it is not blanket permission for later actions. The app-generated resume directive must preserve that distinction so the Coordinator can continue the original objective only up to the next safe boundary.
