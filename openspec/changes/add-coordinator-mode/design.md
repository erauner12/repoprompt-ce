...
Window-level chrome should follow the same rule. When Coordinator mode is the active main surface, the window title should identify the Coordinator surface and workspace rather than the active Agent session tab. Parent/delegate session titles belong in board/list rows, inspector detail, or explicit Agent Mode deep links, not in the top-level Coordinator window title.
