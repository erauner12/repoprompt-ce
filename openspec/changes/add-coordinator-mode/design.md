...
The classifier is the safety line. It holds when proactive follow-through is disabled, when the Coordinator is active, when any owned child is in `Needs you` or `Blocked`, or when the next step needs explicit human permission. It may resume for child terminal states and advisory review packets, but human continuation approval is deliberately visible and conversational rather than hidden in an inspector row action.
