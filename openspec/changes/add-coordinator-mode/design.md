...
Board/list rows should expose a small structured workstream projection so the UI is not forced to infer work from session titles. The projection is a read model derived from existing session/live-state metadata: objective summary, phase, child session ID, owner Coordinator root when present, worktree binding, workflow label, review packet ID, and next action. It is not a separate source of truth and must not mutate runtime state; it makes the current session/worktree/review state legible enough for follow-through and human gates to reason about the same item the user sees. Follow-through classification should consume this projection first, falling back to lower-level row fields only when the projection is unavailable, so the board-visible work item and supervisor decision describe the same phase, review packet, and next action.
...
The classifier is the safety line. It holds when proactive follow-through is disabled, when the Coordinator is active, when any owned child is in `Needs you` or `Blocked`, or when a required human-review packet has not been acknowledged. It may resume for child terminal states, advisory review packets, and cleared continuation gates. The implemented gate types include `reviewRequired`, cleared by `Mark reviewed`, and `actionApprovalRequired`, cleared by a scoped approval surface that names exactly one approved next action such as `continuePlan`, `commitChanges`, `createPullRequest`, or `applyMergePreview`.

`Mark reviewed` means exactly one thing: the human review gate is cleared. It is not approval to apply, merge, commit, push, or bypass any remaining permission prompt. A scoped action approval clears a different gate, but only for the named action; it is not blanket permission for later actions. The app-generated resume directive must preserve that distinction so the Coordinator can continue the original objective only up to the next safe boundary.
