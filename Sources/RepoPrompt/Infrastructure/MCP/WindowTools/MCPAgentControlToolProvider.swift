        ...
        runtime.tool(
            ...
            **Operations**: list | list_missions | doctor | select | new | ensure_mission | start_mission | stop_mission | archive_mission | submit | mission_plan | propose_revision | mission_status | mission_events | receipt | set_pace | set_autonomy | wait_for_update
            ...
            inputSchema: .object(
                ...
                **submit**: message (required unless checkpoint_action is present), coordinator_session_id? or new_parent?, coordinator_model_id? with new_parent, checkpoint_action? (revise_plan|keep_current_plan|proceed|gather_evidence|deepen_plan|independent_critique|start_smaller|stop), expected_checkpoint_instance_id required for every non-stop checkpoint action; proposal actions also require proposal_id and expected_contract_fingerprint.
