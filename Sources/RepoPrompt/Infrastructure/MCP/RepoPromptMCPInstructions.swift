...
enum RepoPromptMCPInstructions {
    ...
    private static func externalMCPText(codeMapsDisabled: Bool) -> String {
        ...
        LONG-RUNNING TOOLS OVER SSE: For remote clients such as OpenClaw, long-running `context_builder` and `oracle_send` calls are ordinary synchronous tool calls carried by the SDK Streamable HTTP SSE transport. Keep the client session open for the tool result, configure a generous MCP call timeout for RepoPrompt (at least 300s, 600s for broad planning/review calls), and use `export_response: true` when you need durable response artifacts. If a client times out first, server-side context/oracle work may already have created workspace state; do not assume cancellation. Recover by inspecting the newest RepoPrompt tab/chat, using `bind_context list` or `workspace_context` to confirm state, then continuing the recovered `context_builder` chat with `oracle_send` when appropriate.
