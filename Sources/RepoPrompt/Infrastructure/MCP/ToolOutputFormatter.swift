...
extension ToolOutputFormatter {
    ...
    static func buildContentBlocks(
        ...
        if isResumableJobEnvelope(result) {
            return formatResumableJobEnvelope(args: args, value: result, emitResources: emitResources)
        }
    ...
    private static func isResumableJobEnvelope(_ value: Value) -> Bool {
        value.objectValue?["kind"]?.stringValue == MCPResumableJobSnapshot.envelopeKind
    }

    private static func formatResumableJobEnvelope(
        args: [String: Value],
        value: Value,
        emitResources: Bool
    ) -> [MCP.Tool.Content] {
        guard let object = value.objectValue,
              object["kind"]?.stringValue == MCPResumableJobSnapshot.envelopeKind
        else {
            return formatGeneric(value: value)
        }

        let tool = object["tool"]?.stringValue ?? "tool"
        let statusRaw = object["status"]?.stringValue ?? "unknown"
        let status = prettifiedResumableJobStatus(statusRaw)
        let jobID = object["job_id"]?.stringValue
        let serverInstanceID = object["server_instance_id"]?.stringValue
        let stage = object["stage"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines)
        let statusText = object["status_text"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines)
        let progress = object["progress_message"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines)
        let wait = object["wait"]?.objectValue
        let icon = resumableJobStatusIcon(statusRaw: statusRaw, waitResult: wait?["result"]?.stringValue)

        var lines: [String] = []
        lines.append("## Resumable MCP Job \(icon)")
        ...
        if let updatedAt = object["updated_at"]?.stringValue, !updatedAt.isEmpty {
            lines.append("- **Updated**: \(updatedAt)")
        }
        if let expiresIn = numericSeconds(object["expires_in_seconds"]) {
            lines.append("- **Expires in**: \(formatSeconds(expiresIn))")
        } else if object["expires_at"]?.stringValue == nil || object["expires_at"] == .null {
            lines.append("- **Expires in**: unavailable")
        }
        ...
        if let wait {
            lines.append(contentsOf: resumableJobWaitLines(wait))
        }
        if let error = object["error"]?.objectValue {
            lines.append(contentsOf: resumableJobErrorLines(error))
        }
        lines.append(contentsOf: resumableJobNextActionLines(object: object, tool: tool))

        var blocks: [MCP.Tool.Content] = [.text(lines.joined(separator: "\n"))]
        if statusRaw == MCPResumableJobStatus.completed.rawValue,
           let nestedResult = object["result"]
        {
            let nestedBlocks = buildContentBlocks(
                toolName: tool,
                args: args,
                result: nestedResult,
    ...
    private static func resumableJobWaitLines(_ wait: [String: Value]) -> [String] {
        let resultRaw = wait["result"]?.stringValue ?? "unknown"
        var line = "- **Wait**: \(prettifiedResumableJobWaitResult(resultRaw))"
        var details: [String] = []
        if let requested = numericSeconds(wait["requested_seconds"]) {
            details.append("requested \(formatSeconds(requested))")
        }
        if let effective = numericSeconds(wait["effective_seconds"]) {
            details.append("effective \(formatSeconds(effective))")
        }
        if !details.isEmpty {
            line += " (\(details.joined(separator: ", ")))"
        }
        var lines = [line]
        if resultRaw == MCPResumableJobWaitMetadata.Result.timedOut.rawValue {
            lines.append("- **Wait result**: timed out; the job is still resumable and was not marked failed.")
        }
        return lines
    }

    private static func resumableJobErrorLines(_ error: [String: Value]) -> [String] {
        var lines: [String] = []
        if let type = error["type"]?.stringValue, !type.isEmpty {
            lines.append("- **Error type**: `\(type)`")
        }
    ...
    private static func resumableJobNextActionLines(object: [String: Value], tool: String) -> [String] {
        ...
        let timeout = max(0, min(25, numericSeconds(object["poll_after_seconds"]) ?? 1))
        var waitObject: [String: Value] = [
            "op": .string("wait"),
            "job_id": .string(jobID),
            "timeout": .double(timeout)
        ]
        var pollObject: [String: Value] = [
            "op": .string("poll"),
            "job_id": .string(jobID)
        ]
        if let serverInstanceID = object["server_instance_id"]?.stringValue, !serverInstanceID.isEmpty {
            waitObject["server_instance_id"] = .string(serverInstanceID)
            pollObject["server_instance_id"] = .string(serverInstanceID)
        }
        if let windowID = object["window_id"]?.intValue {
            waitObject["window_id"] = .int(windowID)
            pollObject["window_id"] = .int(windowID)
        }
        return [
            "- **Next wait call**: `\(tool)` `\(rawJSONString(.object(waitObject)))`",
            "- **Next poll call**: `\(tool)` `\(rawJSONString(.object(pollObject)))`"
        ]
    }

    private static func resumableJobStatusIcon(statusRaw: String, waitResult: String?) -> String {
        if waitResult == MCPResumableJobWaitMetadata.Result.timedOut.rawValue {
            return "⏳"
        }
        switch statusRaw {
        case MCPResumableJobStatus.completed.rawValue:
            return statusIcon(success: true)
        case MCPResumableJobStatus.queued.rawValue,
             MCPResumableJobStatus.running.rawValue,
             MCPResumableJobStatus.cancelling.rawValue:
            return "⏳"
        case MCPResumableJobStatus.cancelled.rawValue,
             MCPResumableJobStatus.expired.rawValue,
             MCPResumableJobStatus.notFound.rawValue,
             MCPResumableJobStatus.serverRestarted.rawValue:
            return statusIcon(success: false, warning: true)
        case MCPResumableJobStatus.failed.rawValue:
            return statusIcon(success: false)
        default:
            return statusIcon(success: nil, warning: true)
        }
    }

    private static func prettifiedResumableJobStatus(_ raw: String) -> String {
        switch raw {
    ...
    private static func prettifiedResumableJobWaitResult(_ raw: String) -> String {
        switch raw {
        case MCPResumableJobWaitMetadata.Result.snapshotReady.rawValue:
            "snapshot ready"
        case MCPResumableJobWaitMetadata.Result.timedOut.rawValue:
            "timed out"
        default:
    ...
    private static func numericSeconds(_ value: Value?) -> Double? {
