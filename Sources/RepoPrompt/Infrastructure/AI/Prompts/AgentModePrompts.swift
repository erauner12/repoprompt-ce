...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - For repo-specific work where the implementation surface is uncertain, make the first visible plan a draft with concrete discovery/grounding nodes. Use `workflow_name:"Investigate"` or `workflow_name:"Deep Plan"` and `execution_policy:"fresh_readonly_child"` on read-only delegated discovery nodes, ask for approval to run discovery, then revise the same Mission Plan with evidence before approving mutable implementation.
        ...
        - For the same workstream, prefer steering the primary child with `agent_run op=steer` after the primary lane exists. For fresh review or independent judgment, start a sibling child on the same worktree. For independent mutable branches, start a fresh worktree.
