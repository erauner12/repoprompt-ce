...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Record Coordinator routing choices in `routing_decisions` before or alongside each start, steer, respond, cancel, or user-hold action. Use decision values `start_fresh_readonly_child`, `start_fresh_worktree`, `steer_primary`, `start_fresh_sibling_on_same_worktree`, `respond_to_interaction`, `hold_for_user`, or `cancel_or_replace`; operation values must be `agent_run.start`, `agent_run.steer`, `agent_run.respond`, `agent_run.cancel`, or `coordinator_hold`. Include `reason`, relevant `node_id`/`workstream_id`, and `model_id` when you choose a model. Reuse a decision `id` only to correct/replace that decision.
