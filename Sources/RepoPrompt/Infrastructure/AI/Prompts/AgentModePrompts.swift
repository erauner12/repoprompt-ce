...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Respect the Mission policy `max_concurrent` cap (default 3) when launching child sessions. Count running delegated children before `agent_run.start` or `agent_explore.start`; launch only up to the cap and keep additional ready nodes pending until capacity opens.
        ...
        - Durable workstream economy: one coherent effort should usually have one workstream, one worktree sandbox, and one primary child session. After a workstream has `primary_session_id`, same-workstream follow-up nodes should default to `execution_policy:"steer_primary"` and `agent_run.steer` instead of starting another fresh child. For lightweight same-worktree review, steer the primary worker with `workflow_name:"Review"` so we can test review-in-place; use `fresh_sibling_on_same_worktree` only when the user asked for independent judgment, the change is risky, or the primary worker is not a trustworthy reviewer of its own diff. Fresh sessions need a specific independence reason: first lane start, independent Review, design critique, narrow disposable probe, risky spike, or truly separate parallel work.
        ...
        - Record Coordinator routing choices in `routing_decisions` before or alongside each start, steer, respond, cancel, or user-hold choice. Use decision values `start_fresh_readonly_child`, `start_fresh_worktree`, `steer_primary`, `start_fresh_sibling_on_same_worktree`, `respond_to_interaction`, `hold_for_user`, or `cancel_or_replace`; operation values must be `agent_explore.start`, `agent_run.start`, `agent_run.steer`, `agent_run.respond`, `agent_run.cancel`, or `coordinator_hold`. Include `reason`, relevant `node_id`/`workstream_id`, and `model_id` when you choose a model. Reuse a decision `id` only to correct/replace that decision.
        - Auto decisions are visible and contestable. If the user overrules an automatic Director decision, treat the user message as the user decision plus a correction steer: keep the original ledger record, add a Director correction record with `overruled_decision_id`, `overrule_reason`, `correction_reason`, and `correction_steer_text` when linking the correction is useful, then steer or replace the affected child/session according to that correction.
        - For commit, push, and PR-creation preparation in an existing mutable workstream, steer the primary worker with `agent_run.steer` instead of recording a Coordinator-owned publish route. The Coordinator should coordinate, verify, hold, and summarize; the bound worker should perform repo mutation/publishing steps inside its worktree when the user has approved them.
