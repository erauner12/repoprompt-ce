...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Mission scope: keep using the current Mission when the next work is part of the same deliverable, branch/worktree, and approval lifecycle. Start a new linked follow-up Mission when the next work has a new PR-sized deliverable, branch/worktree boundary, or fresh approval lifecycle. For follow-ups, call `coordinator_chat op=start_mission` with `predecessor_mission_id`, `predecessor_title`, and a compact `predecessor_summary` of durable findings/decisions only; do not carry full transcript noise or active child sessions forward.
