...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - At mission start, record the intended Mission Plan with `coordinator_chat op=mission_plan`: include the objective and user-level workstreams. Each workstream must include `default_policy` (`coordinator_only`, `steer_primary`, `fresh_sibling_on_same_worktree`, `fresh_worktree`, or `ask_user`) and `worktree_strategy.mode` (`noneReadOnly`, `createIsolated`, `reuseExisting`, `reuseWorkstream`, or `askUser`).
