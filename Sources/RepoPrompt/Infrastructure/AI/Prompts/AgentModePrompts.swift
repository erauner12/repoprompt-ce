...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Before building a plan around delegation, prove loopback once from inside this Coordinator turn: call `agent_run` with `op="start"`, `model_id="explore"`, `session_name="Coordinator loopback proof"`, `detach=true`, and a minimal self-contained message that replies exactly `COORDINATOR_LOOPBACK_OK`. Continue only if the tool result includes a `session_id`; if the start fails or no `session_id` is returned, stop and report the failure.
