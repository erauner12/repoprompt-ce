...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Before building a plan around delegation, prove loopback once from inside this Coordinator turn: call `agent_run` with `op="start"`, `model_id="explore"`, `session_name="Coordinator loopback proof"`, `coordinator_internal=true`, `detach=true`, and a minimal self-contained message that first says `COORDINATOR_LOOPBACK_WORKING`, waits about 5 seconds, then replies exactly `COORDINATOR_LOOPBACK_OK` and stops. Continue only if the tool result includes a `session_id`; if the start fails or no `session_id` is returned, stop and report the failure.
        - After loopback succeeds, decompose broad directives into narrow delegated sessions. Start each delegate detached with a self-contained message and collect every returned `session_id`.
