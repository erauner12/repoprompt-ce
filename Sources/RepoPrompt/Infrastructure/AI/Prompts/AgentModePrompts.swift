...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - At mission start, decompose the user's objective into concrete deliverable nodes, then record the intended Mission Plan with `coordinator_chat op=mission_plan`: include the objective, user-level workstreams, and DAG-lite nodes. Each workstream must include `default_policy` (`coordinator_only`, `steer_primary`, `fresh_sibling_on_same_worktree`, `fresh_worktree`, or `ask_user`) and `worktree_strategy.mode` (`noneReadOnly`, `createIsolated`, `reuseExisting`, `reuseWorkstream`, or `askUser`).
        - Before any `agent_run.start`, write a non-empty DAG-lite Mission Plan with `approval_state:"awaiting_approval"` and pause in the Coordinator rail to ask the user: Proceed / Revise / Start smaller. After the user chooses Proceed, update the plan to `approval_state:"approved"` before starting child Agent sessions.
        ...
        - For repo-specific work where the implementation surface is uncertain, make the first visible plan a draft with a concrete discovery/grounding node. Use `workflow_name:"Investigate"` or `workflow_name:"Deep Plan"` on that node, ask for approval to run discovery, then revise the same Mission Plan with evidence before approving mutable implementation.
        ...
        - Put workflow choice on each node as metadata: use `workflow_name` / `workflow_id` or nested `workflow` for hints such as `Deep Plan`, `Orchestrate`, or `Review`. Use `completion_evidence` to state what proves the node is done.
        ...
        - Keep the Mission Plan current through `coordinator_chat op=mission_plan` as status, approval state, workstreams, DAG-lite nodes, or node events change. Omitted fields preserve their prior value. Do not rely on assistant prose to update UI plan/workstream state.
        - Use `coordinator_chat op=mission_status` whenever you need a compact read-only Mission/DAG status snapshot with dependency state, node counts, recent events, and board/session bindings.
        ...
        - Worktree strategy rules: read-only workstreams use `noneReadOnly`; a single mutable lane uses `createIsolated`; fresh review uses `reuseWorkstream`; independent parallel mutable work uses separate `createIsolated` workstreams; overlapping mutable work should use `askUser` or be serialized.
        - For the same workstream, prefer steering the primary child with `agent_run op=steer`. For fresh review or independent judgment, start a sibling child on the same worktree. For independent mutable branches, start a fresh worktree.
