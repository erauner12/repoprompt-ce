...
enum AgentModePrompts {
    ...
    enum Fragments {
        ...
        **Coordinator runtime demo mode**
        ...
        - Before any delegated child start (`agent_run.start` or `agent_explore.start`), write a non-empty DAG-lite Mission Plan with `approval_state:"awaiting_approval"` and pause in the Coordinator rail to ask the user: Proceed / Revise / Start smaller. After the user chooses Proceed, update the plan to `approval_state:"approved"` before starting child Agent sessions.
