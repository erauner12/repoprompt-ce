...
```bash
.agents/skills/rpce-director-e2e/scripts/director_e2e.py --scenario s2 --workspace homelab-garden --window 1 --sandbox-root /path/to/e2e-sandbox
```
...
- Store artifacts under `tmp/director-e2e-runs/<run-id>/`: command logs, compact/full statuses, report JSON, and receipt summary.
...
- `s2`: parallel fan-out and convergence. Expects two parents to run concurrently, a dependent summary node to wait until parents complete, a ready-node transition before launch, no extra human submit after approval, cap discipline, completion, and exactly `A.md`, `B.md`, `SUMMARY.md` in the sandbox.
