...
final class SystemPromptServiceCoordinatorModeTests: XCTestCase {
    func testCoordinatorPromptOmitsAutoPaceByDefault() {
        ...
        XCTAssertTrue(prompt.contains("Director context contract"))
        XCTAssertTrue(prompt.contains("bounded Mission ledger"))
        XCTAssertTrue(prompt.contains("judgment_bundle"))
        XCTAssertTrue(prompt.contains("probe_answer"))
        XCTAssertTrue(prompt.contains("For visible rail narration"))
        XCTAssertTrue(prompt.contains("the Director can answer routine child questions"))
        XCTAssertTrue(prompt.contains("use raw keys only inside structured `coordinator_chat` payloads"))
        ...
        XCTAssertTrue(prompt.contains("Review nodes use `workflow_name:\"Review\"` whether they steer the primary worker or start a fresh sibling"))
        XCTAssertTrue(prompt.contains("For lightweight same-worktree review, steer the primary worker with `workflow_name:\"Review\"`"))
        ...
        XCTAssertTrue(prompt.contains("Wait for the user-visible approval checkpoint before ordinary delegation"))
        XCTAssertTrue(prompt.contains("Do not set `approval_state:\"approved\"` or `approval_state:\"not_required\"` yourself"))
        XCTAssertTrue(prompt.contains("approved is only produced by the trusted user checkpoint transaction"))
        XCTAssertTrue(prompt.contains("legacy not_required is non-authorizing"))
        XCTAssertTrue(prompt.contains("the approved contract is immutable through generic/runtime `coordinator_chat op=mission_plan` updates"))
        XCTAssertTrue(prompt.contains("do not rewrite the objective, shape, workstreams, node membership/dependencies/workflow/execution policy/done criteria, worktree strategy, policy snapshot, pace, `childAsk`, or autonomy"))
        XCTAssertTrue(prompt.contains("ask for a trusted user-visible revision that creates a fresh revision-bound approval checkpoint"))
        XCTAssertTrue(prompt.contains("coordinator_chat op=propose_revision"))
        XCTAssertTrue(prompt.contains("only when the requested remedy changes the approved material contract"))
        XCTAssertTrue(prompt.contains("structured summary-only proposal"))
        XCTAssertTrue(prompt.contains("Ordinary evidence, failures, tool errors, and changed assumptions remain evidence/failure bookkeeping or explanatory prose"))
        XCTAssertTrue(prompt.contains("unless the requested remedy changes approved contract fields"))
        XCTAssertTrue(prompt.contains("Never directly revise the approved contract"))
        XCTAssertTrue(prompt.contains("resolve your own proposal"))
        XCTAssertTrue(prompt.contains("author or impersonate a user decision"))
        XCTAssertTrue(prompt.contains("treat proposal filing as approval"))
        ...
        XCTAssertTrue(prompt.contains("Mission shapes are free-form runtime hints"))
        XCTAssertTrue(prompt.contains("shape_summary"))
        XCTAssertTrue(prompt.contains("PRD Slices"))
        XCTAssertTrue(prompt.contains("dependent slice after A+B"))
        XCTAssertTrue(prompt.contains("stacked PR"))
        XCTAssertTrue(prompt.contains("max_concurrent"))
        XCTAssertTrue(prompt.contains("default 3"))
        XCTAssertTrue(prompt.contains("ready_node_ids"))
        XCTAssertTrue(prompt.contains("deps_satisfied"))
        XCTAssertTrue(prompt.contains("instead of recomputing from raw node arrays"))
        ...
        XCTAssertTrue(prompt.contains("A `routing_decisions` entry is intent, not launch proof"))
        XCTAssertTrue(prompt.contains("include the returned `bound_session_id` or `bound_interaction_id`"))
        XCTAssertTrue(prompt.contains("replace launch/waiting/bound text with actual result evidence"))
        XCTAssertTrue(prompt.contains("must not keep stale phrases"))
        XCTAssertTrue(prompt.contains("Auto decisions are visible and contestable"))
        XCTAssertTrue(prompt.contains("user decision plus a correction steer"))
        XCTAssertTrue(prompt.contains("overruled_decision_id"))
        XCTAssertTrue(prompt.contains("steer the primary worker with `agent_run.steer`"))
        XCTAssertFalse(prompt.contains("coordinator_publish"))
