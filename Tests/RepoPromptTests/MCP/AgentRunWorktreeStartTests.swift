...
final class AgentRunWorktreeStartTests: AgentRunWorktreeStartGitSeedTestCase {
    ...
    private func makeAgentExploreStartService(
        ...
        AgentExploreMCPToolService(
            ...
            startRun: { target, message, _, agentModeVM, agentRaw, modelRaw, reasoningEffortRaw, taskLabelKind, allowsAgentExternalControlTools, workflow, _, _ in
    ...
    private func makeAgentRunStartService(
        ...
        var service = AgentRunMCPToolService(
            ...
            withHeartbeat: { _, _, _, _, operation in try await operation() },
            startRun: { target, _, _, agentModeVM, agentRaw, modelRaw, reasoningEffortRaw, taskLabelKind, allowsAgentExternalControlTools, workflow, _, _ in
                guard let sessionID = target.sessionID else {
