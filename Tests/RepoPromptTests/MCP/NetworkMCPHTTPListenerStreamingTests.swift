...
final class NetworkMCPHTTPListenerStreamingTests: XCTestCase {
    func testStreamingResponsePreservesHeadersAndWritesOpaqueChunkedSSE() async throws {
        ...
        try await withListener(response: .streaming(headers: [
            ...
            MCPStreamableHTTPHeader.sessionID: "session-stream",
            "X-Local-Header": "kept",
            "Content-Length": "999"
    ...
    private func withListener(
        ...
        let listener = MCPStreamableHTTPListener(configuration: .init(bindAddress: "127.0.0.1", port: 0)) { _ in
            switch response {
            case let .finite(headers, body):
                MCPStreamableHTTPResponse(statusCode: 200, headers: headers, body: .data(body))
            case let .streaming(headers, chunks):
                MCPStreamableHTTPResponse(statusCode: 200, headers: headers, body: .stream(AsyncThrowingStream { continuation in
                    Task {
                        for chunk in chunks {
            ...
            case let .failingStream(chunk):
                MCPStreamableHTTPResponse(statusCode: 200, headers: [
                    "Content-Type": "text/event-stream",
                    "Cache-Control": "no-cache, no-transform",
                    "Connection": "keep-alive"
